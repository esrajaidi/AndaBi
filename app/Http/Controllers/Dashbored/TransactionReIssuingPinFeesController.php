...
use App\Imports\TransactionsReIssuingPinFeesImport;
use App\Models\Branche;
use App\Models\TransactionReIssuingPinFees;
...
class TransactionReIssuingPinFeesController extends Controller
{
    ...
    if ($request->ajax()) {
    $data = TransactionReIssuingPinFees::latest()->get();
    ...
    ->make(true);
       ...
       return view('dashboard.transactionReIssuingPinFees.index')->with('branches',$branches);
}
   ...
   public function uplode(){
        ...
        return view('dashboard.transactionReIssuingPinFees.uplode');
    ...
    public function storeUplode(Request $request)
    {
        ...
        try {
            ...
            DB::transaction(function () use ($request,$fileName) {
                Excel::import(new TransactionsReIssuingPinFeesImport, $request->file('file'));
                          
            });
        
            ActivityLogger::activity($fileName.'تمت عملية  تحميل ملف Transactions ReIssuing Pin Fees  بنجاح');

            Alert::success('تمت عملية  تحميل ملف  بنجاح');

            return redirect('transaction_re_issuing_pin_fees');
        } catch (\Exception $e) {
            Alert::warning($e->getMessage());
            dd($e->getMessage());
            ActivityLogger::activity( $e->getMessage().' فشل   تحميل ملف Transactions ReIssuing Pin Fees  ');
    ...
    { 
        ...
        ->get();
            ...
            $query = DB::table('transaction_re_issuing_pin_fees')
            ->select(DB::raw('
        ...
        return view('dashboard.transactionReIssuingPinFees.report_bybranche')
        ->with('data',$data)
    ...
    {
        $fileName="TransactionReIssuingPinFees_bybranche_".str_replace( array( '\'', '/',"-" ), '', Now()->toDateString()).".pdf";
        $title='Transaction ReIssuing Fees Report Branche';
    ...
    {       
        ...
        $fileName="TransactionReIssuingPinFees_bybranche_".str_replace( array( '\'', '/',"-" ), '', Now()->toDateString()).".xlsx";
        ActivityLogger::activity($fileName. "تم تصدير ملف  تحت اسم ");
     ...
     */
     ...
     { 
         ...
         return view('dashboard.transactionReIssuingPinFees.report')
         ->with('data',$data);
     ...
     {
        $fileName="TransactionReIssuingPinFees_".str_replace( array( '\'', '/',"-" ), '', Now()->toDateString()).".pdf";
        $title='Transaction ReIssuing Fees ';
        ActivityLogger::activity($fileName. "تم تصدير ملف  تحت اسم ");
     ...
     {       
         ...
         $fileName="TransactionReIssuingPinFees_".str_replace( array( '\'', '/',"-" ), '', Now()->toDateString()).".xlsx";
         ActivityLogger::activity($fileName. "تم تصدير ملف  تحت اسم ");
