...
use App\Models\TransactionMasterATM;
...
use App\Imports\TransactionsMatserATMImport;

class TransactionMasterATMController extends Controller
{
    ...
    if ($request->ajax()) {
    $data = TransactionMasterATM::latest()->get();
    ...
    ->make(true);
       ...
       return view('dashboard.transactionMasterATM.index')->with('branches',$branches);
}
   ...
   public function uplode(){
        ...
        return view('dashboard.transactionMasterATM.uplode');
    ...
    public function storeUplode(Request $request)
    {
        ...
        try {
            ...
            DB::transaction(function () use ($request,$fileName) {
                Excel::import(new TransactionsMatserATMImport, $request->file('file'));
                          
            });
            
            ActivityLogger::activity($fileName.'تمت عملية  تحميل ملف Transactions Master ATM  بنجاح');

            Alert::success('تمت عملية  تحميل ملف  بنجاح');

            return redirect('transaction_master_a_t_m_s');
        } catch (\Exception $e) {
            Alert::warning($e->getMessage());
            ActivityLogger::activity( $e->getMessage().' فشل   تحميل ملف Transactions Master ATM  ');
    ...
    { 
        ...
        ->get();
            ...
            $query = DB::table('transaction_master_a_t_m_s')
            ->select(DB::raw('
        ...
        return view('dashboard.transactionMasterATM.report_bybranche')
        ->with('data',$data)
    ...
    {
        $fileName="TransactionMasterATM_bybranche_".str_replace( array( '\'', '/',"-" ), '', Now()->toDateString()).".pdf";
        $title='Transaction Matser ATM Report Branche';
    ...
    {       
        ...
        $fileName="TransactionMasterATM_bybranche_".str_replace( array( '\'', '/',"-" ), '', Now()->toDateString()).".xlsx";
        ActivityLogger::activity($fileName. "تم تصدير ملف  تحت اسم ");
     ...
     */
     ...
     { 
         ...
         return view('dashboard.transactionMasterATM.report')
         ->with('data',$data);
     ...
     {
        $fileName="TransactionMasterATM_".str_replace( array( '\'', '/',"-" ), '', Now()->toDateString()).".pdf";
        $title='Transaction Matser ATM ';
        ActivityLogger::activity($fileName. "تم تصدير ملف  تحت اسم ");
     ...
     {       
         ...
         $fileName="TransactionMasterATM_".str_replace( array( '\'', '/',"-" ), '', Now()->toDateString()).".xlsx";
         ActivityLogger::activity($fileName. "تم تصدير ملف  تحت اسم ");
