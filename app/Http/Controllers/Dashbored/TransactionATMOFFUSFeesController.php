...
use App\Imports\TransactionsATMOFFUSFeesImport;
use App\Models\Branche;
use App\Models\TransactionATMOFFUSFees;
...
class TransactionATMOFFUSFeesController extends Controller
{
    ...
    if ($request->ajax()) {
    $data = TransactionATMOFFUSFees::latest()->get();
    ...
    ->make(true);
       ...
       return view('dashboard.transactionATMOFFUSFees.index')->with('branches',$branches);
}
   ...
   public function uplode(){
        ...
        return view('dashboard.transactionATMOFFUSFees.uplode');
    ...
    public function storeUplode(Request $request)
    {
        ...
        try {
            ...
            DB::transaction(function () use ($request,$fileName) {
                Excel::import(new TransactionsATMOFFUSFeesImport, $request->file('file'));
                          
            });
            
            ActivityLogger::activity($fileName.'تمت عملية  تحميل ملف Transactions ATMOFFUSFees  بنجاح');

            Alert::success('تمت عملية  تحميل ملف  بنجاح');

            return redirect('transaction_a_t_m_o_f_f_u_s_fees');
        } catch (\Exception $e) {
            Alert::warning($e->getMessage());
            dd($e->getMessage());
            ActivityLogger::activity( $e->getMessage().' فشل   تحميل ملف Transactions ATMOFFUSFees  ');
    ...
    { 
        ...
        ->get();
            ...
            $query = DB::table('transaction_a_t_m_o_f_f_u_s_fees')
            ->select(DB::raw('
        ...
        return view('dashboard.transactionATMOFFUSFees.report_bybranche')
        ->with('data',$data)
    ...
    {
        $fileName="transactionATMOFFUSFees_bybranche_".str_replace( array( '\'', '/',"-" ), '', Now()->toDateString()).".pdf";
        $title='Transaction ATMOFFUSFees Report Branche';
    ...
    {       
        ...
        $fileName="transactionATMOFFUSFees_bybranche_".str_replace( array( '\'', '/',"-" ), '', Now()->toDateString()).".xlsx";
        ActivityLogger::activity($fileName. "تم تصدير ملف  تحت اسم ");
     ...
     */
     ...
     { 
         ...
         return view('dashboard.transactionATMOFFUSFees.report')
         ->with('data',$data);
     ...
     {
        $fileName="transactionATMOFFUSFees_".str_replace( array( '\'', '/',"-" ), '', Now()->toDateString()).".pdf";
        $title='Transaction ATMOFFUSFees ';
        ActivityLogger::activity($fileName. "تم تصدير ملف  تحت اسم ");
     ...
     {       
         ...
         $fileName="transactionATMOFFUSFees_".str_replace( array( '\'', '/',"-" ), '', Now()->toDateString()).".xlsx";
         ActivityLogger::activity($fileName. "تم تصدير ملف  تحت اسم ");
