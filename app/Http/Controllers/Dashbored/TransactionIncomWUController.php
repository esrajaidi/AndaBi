...
use App\Imports\TransactionsIncomWUImport;
use App\Models\Branche;
use App\Models\TransactionIncomWU;
...
class TransactionIncomWUController extends Controller
{
    ...
    if ($request->ajax()) {
    $data = TransactionIncomWU::latest()->get();
    ...
    ->make(true);
       ...
       return view('dashboard.transactionIncomWU.index')->with('branches',$branches);
}
   ...
   public function uplode(){
        ...
        return view('dashboard.transactionIncomWU.uplode');
    ...
    public function storeUplode(Request $request)
    {
        ...
        try {
            ...
            DB::transaction(function () use ($request,$fileName) {
                Excel::import(new TransactionsIncomWUImport, $request->file('file'));
                          
            });
            
            ActivityLogger::activity($fileName.'تمت عملية  تحميل ملف Transactions Incom  WU  بنجاح');

            Alert::success('تمت عملية  تحميل ملف  بنجاح');

            return redirect('transaction_incom_w_u_s');
        } catch (\Exception $e) {
            Alert::warning($e->getMessage());
            dd($e->getMessage());
            ActivityLogger::activity( $e->getMessage().' فشل   تحميل ملف Transactions Incom WU  ');
    ...
    { 
        ...
        ->get();
            ...
            $query = DB::table('transaction_incom_w_u_s')
            ->select(DB::raw('
        ...
        return view('dashboard.transactionIncomWU.report_bybranche')
        ->with('data',$data)
    ...
    {
        $fileName="transaction_incomWU_bybranche_".str_replace( array( '\'', '/',"-" ), '', Now()->toDateString()).".pdf";
        $title='Transaction Incom WU Report Branche';
    ...
    {       
        ...
        $fileName="transaction_incomWU_bybranche_".str_replace( array( '\'', '/',"-" ), '', Now()->toDateString()).".xlsx";
        ActivityLogger::activity($fileName. "تم تصدير ملف  تحت اسم ");
     ...
     */
     ...
     { 
         ...
         return view('dashboard.transactionIncomWU.report')
         ->with('data',$data);
     ...
     {
        $fileName="transaction_incomWU_".str_replace( array( '\'', '/',"-" ), '', Now()->toDateString()).".pdf";
        $title='Transaction Incom WU ';
        ActivityLogger::activity($fileName. "تم تصدير ملف  تحت اسم ");
     ...
     {       
         ...
         $fileName="transaction_incomWU_".str_replace( array( '\'', '/',"-" ), '', Now()->toDateString()).".xlsx";
         ActivityLogger::activity($fileName. "تم تصدير ملف  تحت اسم ");
