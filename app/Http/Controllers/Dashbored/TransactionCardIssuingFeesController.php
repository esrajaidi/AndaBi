...
class TransactionCardIssuingFeesController extends Controller
{
    ...
    if ($request->ajax()) {
    $data = TransactionCardIssuingFees::latest()->get();
    ...
    ->make(true);
       ...
       return view('dashboard.transactionCardIssuingFees.index')->with('branches',$branches);
}
   ...
   public function uplode(){
        ...
        return view('dashboard.transactionCardIssuingFees.uplode');
    ...
    public function storeUplode(Request $request)
    {
        ...
        try {
            ...
            ActivityLogger::activity($fileName.'تمت عملية  تحميل ملف Transactions CardIssuingFees  بنجاح');

            Alert::success('تمت عملية  تحميل ملف  بنجاح');

            return redirect('transaction_card_issuing_fees');
        } catch (\Exception $e) {
            Alert::warning($e->getMessage());
            dd($e->getMessage());
            ActivityLogger::activity( $e->getMessage().' فشل   تحميل ملف Transactions CardIssuingFees  ');
    ...
    { 
        ...
        ->get();
            ...
            $query = DB::table('transaction_card_issuing_fees')
            ->select(DB::raw('
        ...
        return view('dashboard.transactionCardIssuingFees.report_bybranche')
        ->with('data',$data)
     ...
     */
     ...
     { 
         ...
         return view('dashboard.transactionCardIssuingFees.report')
         ->with('data',$data);
