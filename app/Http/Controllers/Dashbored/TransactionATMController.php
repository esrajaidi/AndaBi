...
use App\Imports\TransactionsATMImport;

class TransactionATMController extends Controller
{
    ...
    if ($request->ajax()) {
    $data = TransactionATM::latest()->get();
   ...
   public function uplode(){
        ...
        return view('dashboard.transactionATM.uplode');
    ...
    public function storeUplode(Request $request)
    {
        ...
        try {
            ...
            DB::transaction(function () use ($request,$fileName) {
                Excel::import(new TransactionsATMImport, $request->file('file'));
                          
            });
            
            ActivityLogger::activity($fileName.'تمت عملية  تحميل ملف Transactions ATM  بنجاح');

            Alert::success('تمت عملية  تحميل ملف  بنجاح');

            return redirect('transaction_a_t_m_s');
        } catch (\Exception $e) {
            Alert::warning($e->getMessage());
            ActivityLogger::activity( $e->getMessage().' فشل   تحميل ملف Transactions ATM  ');
    ...
    { 
        ...
        ->get();
            ...
            $query = DB::table('transaction_a_t_m_s')
            ->select(DB::raw('
        ...
        return view('dashboard.transactionATM.report_bybranche')
        ->with('data',$data)
    ...
    {
        $fileName="TransactionATM_bybranche_".str_replace( array( '\'', '/',"-" ), '', Now()->toDateString()).".pdf";
        $title='Transaction ATM Report Branche';
    ...
    {       
        ...
        $fileName="TransactionATM_bybranche_".str_replace( array( '\'', '/',"-" ), '', Now()->toDateString()).".xlsx";
        ActivityLogger::activity($fileName. "تم تصدير ملف  تحت اسم ");
     ...
     */
     ...
     { 
         ...
         return view('dashboard.transactionATM.report')
         ->with('data',$data);
     ...
     {
        $fileName="TransactionATM_".str_replace( array( '\'', '/',"-" ), '', Now()->toDateString()).".pdf";
        $title='Transaction ATM ';
        ActivityLogger::activity($fileName. "تم تصدير ملف  تحت اسم ");
     ...
     {       
         ...
         $fileName="TransactionATM_".str_replace( array( '\'', '/',"-" ), '', Now()->toDateString()).".xlsx";
         ActivityLogger::activity($fileName. "تم تصدير ملف  تحت اسم ");
