...
use App\Imports\TransactionWUPurchaseCommissionImport;
use App\Models\Branche;
use App\Models\TransactionWUPurchaseCommission;
...
class TransactionWUPurchaseCommissionController extends Controller
{
    ...
    if ($request->ajax()) {
    $data = TransactionWUPurchaseCommission::latest()->get();
    ...
    ->make(true);
       ...
       return view('dashboard.transactionWUPurchaseCommission.index')->with('branches',$branches);
}
   ...
   public function uplode(){
        ...
        return view('dashboard.transactionWUPurchaseCommission.uplode');
    ...
    public function storeUplode(Request $request)
    {
        ...
        try {
            ...
            DB::transaction(function () use ($request,$fileName) {
                Excel::import(new TransactionWUPurchaseCommissionImport, $request->file('file'));
                          
            });
            
            ActivityLogger::activity($fileName.'تمت عملية  تحميل ملف Transaction WU Purchase Commission  بنجاح');

            Alert::success('تمت عملية  تحميل ملف  بنجاح');

            return redirect('transaction_w_u_purchase_commissions');
        } catch (\Exception $e) {
            Alert::warning($e->getMessage());
            dd($e->getMessage());
            ActivityLogger::activity( $e->getMessage().' فشل   تحميل ملف Transaction WU Purchase Commission  ');
    ...
    { 
        ...
        ->get();
            ...
            $query = DB::table('transaction_w_u_purchase_commissions')
            ->select(DB::raw('
        ...
        return view('dashboard.transactionWUPurchaseCommission.report_bybranche')
        ->with('data',$data)
    ...
    {
        $fileName="transaction_w_u_purchase_commissions_bybranche_".str_replace( array( '\'', '/',"-" ), '', Now()->toDateString()).".pdf";
        $title='Transaction WU Purchase Commission Report Branche';
    ...
    {       
        ...
        $fileName="transaction_w_u_purchase_commissions_bybranche_".str_replace( array( '\'', '/',"-" ), '', Now()->toDateString()).".xlsx";
        ActivityLogger::activity($fileName. "تم تصدير ملف  تحت اسم ");
     ...
     */
     ...
     { 
         ...
         return view('dashboard.transactionWUPurchaseCommission.report')
         ->with('data',$data);
     ...
     {
        $fileName="transaction_w_u_purchase_commissions_".str_replace( array( '\'', '/',"-" ), '', Now()->toDateString()).".pdf";
        $title='Transaction WU Purchase Commission ';
        ActivityLogger::activity($fileName. "تم تصدير ملف  تحت اسم ");
     ...
     {       
         ...
         $fileName="transaction_w_u_purchase_commissions_".str_replace( array( '\'', '/',"-" ), '', Now()->toDateString()).".xlsx";
         ActivityLogger::activity($fileName. "تم تصدير ملف  تحت اسم ");
