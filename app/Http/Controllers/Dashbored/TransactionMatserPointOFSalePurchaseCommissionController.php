...
use App\Models\TransactionMatserPointOFSalePurchaseCommission;
...
class TransactionMatserPointOFSalePurchaseCommissionController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        if ($request->ajax()) {
        $data = TransactionMatserPointOFSalePurchaseCommission::latest()->get();
       ...
       return view('dashboard.transactionMatserPointOFSalePurchaseCommission.index')->with('branches',$branches);
}
   ...
   public function uplode(){
        ...
        return view('dashboard.transactionMatserPointOFSalePurchaseCommission.uplode');
    ...
    public function storeUplode(Request $request)
    {
        ...
        try {
            ...
            DB::transaction(function () use ($request,$fileName) {
                Excel::import(new TransactionMatserPointOFSalePurchaseCommissionImport, $request->file('file'));
                          
            });
            
            ActivityLogger::activity($fileName.'تمت عملية  تحميل ملف Transaction Matser Point OF Sale Purchase Commission  بنجاح');

            Alert::success('تمت عملية  تحميل ملف  بنجاح');

            return redirect('transaction_matser_point_o_f_sale_purchase_commissions');
        } catch (\Exception $e) {
            Alert::warning($e->getMessage());
            dd($e->getMessage());
            ActivityLogger::activity( $e->getMessage().' فشل   تحميل ملف Transaction Matser Point OF Sale Purchase Commission  ');
    ...
    { 
        ...
        ->get();
            ...
            $query = DB::table('transaction_matser_point_o_f_sale_purchase_commissions')
            ->select(DB::raw('
        ...
        return view('dashboard.transactionMatserPointOFSalePurchaseCommission.report_bybranche')
        ->with('data',$data)
    ...
    {
        $fileName="transactionMatserPointOFSalePurchaseCommission_bybranche_".str_replace( array( '\'', '/',"-" ), '', Now()->toDateString()).".pdf";
        $title='Transaction Matser Point OF Sale Purchase Commission Report Branche';
    ...
    {       
        ...
        $fileName="transactionMatserPointOFSalePurchaseCommission_bybranche_".str_replace( array( '\'', '/',"-" ), '', Now()->toDateString()).".xlsx";
        ActivityLogger::activity($fileName. "تم تصدير ملف  تحت اسم ");
     ...
     */
     ...
     { 
         ...
         return view('dashboard.transactionMatserPointOFSalePurchaseCommission.report')
         ->with('data',$data);
     ...
     {
        $fileName="transactionMatserPointOFSalePurchaseCommission_".str_replace( array( '\'', '/',"-" ), '', Now()->toDateString()).".pdf";
        $title='Transaction Matser Point OF Sale Purchase Commission ';
        ActivityLogger::activity($fileName. "تم تصدير ملف  تحت اسم ");
     ...
     {       
         ...
         $fileName="transactionMatserPointOFSalePurchaseCommission_".str_replace( array( '\'', '/',"-" ), '', Now()->toDateString()).".xlsx";
         ActivityLogger::activity($fileName. "تم تصدير ملف  تحت اسم");
