...
use App\Imports\TransactionsCardReIssuingFeesImport;
use App\Models\Branche;
use App\Models\TransactionCardReIssuingFees;
...
class TransactionCardReIssuingFeesController extends Controller
{
    ...
    if ($request->ajax()) {
    $data = TransactionCardReIssuingFees::latest()->get();
    ...
    ->make(true);
       ...
       return view('dashboard.transactionCardReIssuingFees.index')->with('branches',$branches);
}
   ...
   public function uplode(){
        ...
        return view('dashboard.transactionCardReIssuingFees.uplode');
    ...
    public function storeUplode(Request $request)
    {
        ...
        try {
            ...
            DB::transaction(function () use ($request,$fileName) {
                Excel::import(new TransactionsCardReIssuingFeesImport, $request->file('file'));
                          
            });
            
            ActivityLogger::activity($fileName.'تمت عملية  تحميل ملف Transactions CardReIssuingFees  بنجاح');

            Alert::success('تمت عملية  تحميل ملف  بنجاح');

            return redirect('transaction_card_re_issuing_fees');
        } catch (\Exception $e) {
            Alert::warning($e->getMessage());
            dd($e->getMessage());
            ActivityLogger::activity( $e->getMessage().' فشل   تحميل ملف Transactions CardReIssuingFees  ');
    ...
    { 
        ...
        ->get();
            ...
            $query = DB::table('transaction_card_re_issuing_fees')
            ->select(DB::raw('
        ...
        return view('dashboard.transactionCardReIssuingFees.report_bybranche')
        ->with('data',$data)
    ...
    {
        $fileName="transactionCardReIssuingFees_bybranche_".str_replace( array( '\'', '/',"-" ), '', Now()->toDateString()).".pdf";
        $title='Transaction CardReIssuingFees Report Branche';
    ...
    {       
        ...
        $fileName="transactionCardReIssuingFees_bybranche_".str_replace( array( '\'', '/',"-" ), '', Now()->toDateString()).".xlsx";
        ActivityLogger::activity($fileName. "تم تصدير ملف  تحت اسم ");
     ...
     */
     ...
     { 
         ...
         return view('dashboard.transactionCardReIssuingFees.report')
         ->with('data',$data);
     ...
     {
        $fileName="transactionCardReIssuingFees_".str_replace( array( '\'', '/',"-" ), '', Now()->toDateString()).".pdf";
        $title='Transaction CardReIssuingFees ';
        ActivityLogger::activity($fileName. "تم تصدير ملف  تحت اسم ");
     ...
     {       
         ...
         $fileName="transactionCardReIssuingFees_".str_replace( array( '\'', '/',"-" ), '', Now()->toDateString()).".xlsx";
         ActivityLogger::activity($fileName. "تم تصدير ملف  تحت اسم ");
