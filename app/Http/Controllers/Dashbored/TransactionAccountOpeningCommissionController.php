...
use App\Imports\TransactionAccountOpeningCommissionImport;
...
class TransactionAccountOpeningCommissionController extends Controller
{
    ...
    if ($request->ajax()) {
    $data = TransactionAccountOpeningCommission::latest()->get();
    ...
    ->make(true);
       ...
       return view('dashboard.transactionAccountOpeningCommission.index')->with('branches',$branches);
}
   ...
   public function uplode(){
        ...
        return view('dashboard.transactionAccountOpeningCommission.uplode');
    ...
    public function storeUplode(Request $request)
    {
        ...
        try {
            ...
            DB::transaction(function () use ($request,$fileName) {
                Excel::import(new TransactionAccountOpeningCommissionImport, $request->file('file'));
                          
            });
            
            ActivityLogger::activity($fileName.'تمت عملية  تحميل ملف Transaction Account Opening Commission  بنجاح');

            Alert::success('تمت عملية  تحميل ملف  بنجاح');

            return redirect('transaction_account_opening_commissions');
        } catch (\Exception $e) {
            Alert::warning($e->getMessage());
            dd($e->getMessage());
            ActivityLogger::activity( $e->getMessage().' فشل   تحميل ملف Transaction Account Opening Commission ');
    ...
    { 
        ...
        ->get();
            ...
            $query = DB::table('transaction_account_opening_commissions')
            ->select(DB::raw('
        ...
        return view('dashboard.transactionAccountOpeningCommission.report_bybranche')
        ->with('data',$data)
    ...
    {
        $fileName="transaction_account_opening_commissions_bybranche_".str_replace( array( '\'', '/',"-" ), '', Now()->toDateString()).".pdf";
        $title='Transaction Account Opening Commission Report Branche';
    ...
    {       
        ...
        $fileName="transaction_account_opening_commissions_bybranche_".str_replace( array( '\'', '/',"-" ), '', Now()->toDateString()).".xlsx";
        ActivityLogger::activity($fileName. "تم تصدير ملف  تحت اسم ");
     ...
     */
     ...
     { 
         ...
         return view('dashboard.transactionAccountOpeningCommission.report')
         ->with('data',$data);
     ...
     {
        $fileName="transaction_account_opening_commissions_".str_replace( array( '\'', '/',"-" ), '', Now()->toDateString()).".pdf";
        $title='Transaction Account Opening Commission ';
        ActivityLogger::activity($fileName. "تم تصدير ملف  تحت اسم ");
     ...
     {       
         ...
         $fileName="transaction_account_opening_commissions_".str_replace( array( '\'', '/',"-" ), '', Now()->toDateString()).".xlsx";
         ActivityLogger::activity($fileName. "تم تصدير ملف  تحت اسم ");
