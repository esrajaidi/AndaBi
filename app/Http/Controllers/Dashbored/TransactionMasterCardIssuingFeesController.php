...
use App\Imports\TransactionMasterCardIssuingFeesImport;
...
class TransactionMasterCardIssuingFeesController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        if ($request->ajax()) {
        $data = TransactionMasterCardIssuingFees::latest()->get();
       ...
       return view('dashboard.transactionMasterCardIssuingFees.index')->with('branches',$branches);
}
   ...
   public function uplode(){
        ...
        return view('dashboard.transactionMasterCardIssuingFees.uplode');
    ...
    public function storeUplode(Request $request)
    {
        ...
        try {
            ...
            DB::transaction(function () use ($request,$fileName) {
                Excel::import(new TransactionMasterCardIssuingFeesImport, $request->file('file'));
                          
            });
            
            ActivityLogger::activity($fileName.'تمت عملية  تحميل ملف Transaction Master Card Issuing Fees  بنجاح');

            Alert::success('تمت عملية  تحميل ملف  بنجاح');

            return redirect('transaction_master_card_issuing_fees');
        } catch (\Exception $e) {
            Alert::warning($e->getMessage());
            dd($e->getMessage());
            ActivityLogger::activity( $e->getMessage().' فشل   تحميل ملف Transactio nMaster Card Issuing Fees  ');
    ...
    { 
        ...
        ->get();
            ...
            $query = DB::table('transaction_master_card_issuing_fees')
            ->select(DB::raw('
        ...
        return view('dashboard.transactionMasterCardIssuingFees.report_bybranche')
        ->with('data',$data)
    ...
    {
        $fileName="transactionMasterCardIssuingFees_bybranche_".str_replace( array( '\'', '/',"-" ), '', Now()->toDateString()).".pdf";
        $title='Transaction MasterCard Issuing Fees Report Branche';
    ...
    {       
        ...
        $fileName="transactionMasterCardIssuingFees_bybranche_".str_replace( array( '\'', '/',"-" ), '', Now()->toDateString()).".xlsx";
        ActivityLogger::activity($fileName. "تم تصدير ملف  تحت اسم ");
     ...
     */
     ...
     { 
         ...
         return view('dashboard.transactionMasterCardIssuingFees.report')
         ->with('data',$data);
     ...
     {
        $fileName="transactionMasterCardIssuingFees_".str_replace( array( '\'', '/',"-" ), '', Now()->toDateString()).".pdf";
        $title='Transaction MasterCard Issuing Fees ';
        ActivityLogger::activity($fileName. "تم تصدير ملف  تحت اسم ");
     ...
     {       
         ...
         $fileName="transactionMasterCardIssuingFees_".str_replace( array( '\'', '/',"-" ), '', Now()->toDateString()).".xlsx";
         ActivityLogger::activity($fileName. "تم تصدير ملف  تحت اسم");
