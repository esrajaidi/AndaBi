...
use App\Imports\TransactionMasterCardMangmentFeesImport;
use App\Models\Branche;
use App\Models\TransactionMasterCardMangmentFees;
...
class TransactionMasterCardMangmentFeesController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        if ($request->ajax()) {
        $data = TransactionMasterCardMangmentFees::latest()->get();
       ...
       return view('dashboard.transactionMasterCardMangmentFees.index')->with('branches',$branches);
}
   ...
   public function uplode(){
        ...
        return view('dashboard.transactionMasterCardMangmentFees.uplode');
    ...
    public function storeUplode(Request $request)
    {
        ...
        try {
            ...
            DB::transaction(function () use ($request,$fileName) {
                Excel::import(new TransactionMasterCardMangmentFeesImport, $request->file('file'));
                          
            });
            
            ActivityLogger::activity($fileName.'تمت عملية  تحميل ملف Transaction Master Card Mangment  Fees  بنجاح');

            Alert::success('تمت عملية  تحميل ملف  بنجاح');

            return redirect('transaction_master_card_mangment_fees');
        } catch (\Exception $e) {
            Alert::warning($e->getMessage());
            dd($e->getMessage());
            ActivityLogger::activity( $e->getMessage().' فشل   تحميل ملف Transactio nMaster Card Mangment Fees  ');
    ...
    { 
        ...
        ->get();
            ...
            $query = DB::table('transaction_master_card_mangment_fees')
            ->select(DB::raw('
        ...
        return view('dashboard.transactionMasterCardMangmentFees.report_bybranche')
        ->with('data',$data)
    ...
    {
        $fileName="transactionMasterCardMangmentFees_bybranche_".str_replace( array( '\'', '/',"-" ), '', Now()->toDateString()).".pdf";
        $title='Transaction MasterCard Mangment Fees Report Branche';
    ...
    {       
        ...
        $fileName="transactionMasterCardMangmentFees_bybranche_".str_replace( array( '\'', '/',"-" ), '', Now()->toDateString()).".xlsx";
        ActivityLogger::activity($fileName. "تم تصدير ملف  تحت اسم ");
     ...
     */
     ...
     { 
         ...
         return view('dashboard.transactionMasterCardMangmentFees.report')
         ->with('data',$data);
     ...
     {
        $fileName="transactionMasterCardMangmentFees_".str_replace( array( '\'', '/',"-" ), '', Now()->toDateString()).".pdf";
        $title='Transaction MasterCard Mangment Fees ';
        ActivityLogger::activity($fileName. "تم تصدير ملف  تحت اسم ");
     ...
     {       
         ...
         $fileName="transactionMasterCardMangmentFees_".str_replace( array( '\'', '/',"-" ), '', Now()->toDateString()).".xlsx";
         ActivityLogger::activity($fileName. "تم تصدير ملف  تحت اسم");
