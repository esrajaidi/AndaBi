...
use App\Imports\TransactionsIncomCardFeesImport;
use App\Models\Branche;
use App\Models\TransactionIncomCardFees;
...
class TransactionIncomCardFeesController extends Controller
{
    ...
    if ($request->ajax()) {
    $data = TransactionIncomCardFees::latest()->get();
    ...
    ->make(true);
       ...
       return view('dashboard.transactionIncomCardFees.index')->with('branches',$branches);
}
   ...
   public function uplode(){
        ...
        return view('dashboard.transactionIncomCardFees.uplode');
    ...
    public function storeUplode(Request $request)
    {
        ...
        try {
            ...
            DB::transaction(function () use ($request,$fileName) {
                Excel::import(new TransactionsIncomCardFeesImport, $request->file('file'));
                          
            });
            
            ActivityLogger::activity($fileName.'تمت عملية  تحميل ملف Transactions Income Card Fees  بنجاح');

            Alert::success('تمت عملية  تحميل ملف  بنجاح');

            return redirect('transaction_incom_card_fees');
        } catch (\Exception $e) {
            Alert::warning($e->getMessage());
            dd($e->getMessage());
            ActivityLogger::activity( $e->getMessage().' فشل   تحميل ملف Transactions Income Card Fees  ');
    ...
    { 
        ...
        ->get();
            ...
            $query = DB::table('transaction_incom_card_fees')
            ->select(DB::raw('
        ...
        return view('dashboard.transactionIncomCardFees.report_bybranche')
        ->with('data',$data)
    ...
    {
        $fileName="transactionIncomCardFees_bybranche_".str_replace( array( '\'', '/',"-" ), '', Now()->toDateString()).".pdf";
        $title='Transaction Income Card Fees Report Branche';
    ...
    {       
        ...
        $fileName="transactionIncomCardFees_bybranche_".str_replace( array( '\'', '/',"-" ), '', Now()->toDateString()).".xlsx";
        ActivityLogger::activity($fileName. "تم تصدير ملف  تحت اسم ");
     ...
     */
     ...
     { 
         ...
         return view('dashboard.transactionIncomCardFees.report')
         ->with('data',$data);
     ...
     {
        $fileName="transactionIncomCardFees_".str_replace( array( '\'', '/',"-" ), '', Now()->toDateString()).".pdf";
        $title='Transaction Income Card Fees ';
        ActivityLogger::activity($fileName. "تم تصدير ملف  تحت اسم ");
     ...
     {       
         ...
         $fileName="transactionIncomCardFees_".str_replace( array( '\'', '/',"-" ), '', Now()->toDateString()).".xlsx";
         ActivityLogger::activity($fileName. "تم تصدير ملف  تحت اسم ");
