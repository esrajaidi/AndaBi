...
use App\Imports\TransactionsWUSMSImport;
use App\Models\Branche;
use App\Models\TransactionWUSMS;
...
class TransactionWUSMSController extends Controller
{
    ...
    if ($request->ajax()) {
    $data = TransactionWUSMS::latest()->get();
    ...
    ->make(true);
       ...
       return view('dashboard.transactionWUSMS.index')->with('branches',$branches);
}
   ...
   public function uplode(){
        ...
        return view('dashboard.transactionWUSMS.uplode');
    ...
    public function storeUplode(Request $request)
    {
        ...
        try {
            ...
            DB::transaction(function () use ($request,$fileName) {
                Excel::import(new TransactionsWUSMSImport, $request->file('file'));
                          
            });
            
            ActivityLogger::activity($fileName.'تمت عملية  تحميل ملف Transactions WU SMS  بنجاح');

            Alert::success('تمت عملية  تحميل ملف  بنجاح');

            return redirect('transaction_w_u_s_m_s');
        } catch (\Exception $e) {
            Alert::warning($e->getMessage());
            dd($e->getMessage());
            ActivityLogger::activity( $e->getMessage().' فشل   تحميل ملف Transactions  WU SMS ');
    ...
    { 
        ...
        ->get();
            ...
            $query = DB::table('transaction_w_u_s_m_s')
            ->select(DB::raw('
        ...
        return view('dashboard.transactionWUSMS.report_bybranche')
        ->with('data',$data)
    ...
    {
        $fileName="transaction_WU_SMS_bybranche_".str_replace( array( '\'', '/',"-" ), '', Now()->toDateString()).".pdf";
        $title='Transaction WU SMS Report Branche';
    ...
    {       
        ...
        $fileName="transaction_WU_SMS_bybranche_".str_replace( array( '\'', '/',"-" ), '', Now()->toDateString()).".xlsx";
        ActivityLogger::activity($fileName. "تم تصدير ملف  تحت اسم ");
     ...
     */
     ...
     { 
         ...
         return view('dashboard.transactionWUSMS.report')
         ->with('data',$data);
     ...
     {
        $fileName="transaction_WU_SMS_".str_replace( array( '\'', '/',"-" ), '', Now()->toDateString()).".pdf";
        $title='Transaction WU SMS ';
        ActivityLogger::activity($fileName. "تم تصدير ملف  تحت اسم ");
     ...
     {       
         ...
         $fileName="transaction_WU_SMS_".str_replace( array( '\'', '/',"-" ), '', Now()->toDateString()).".xlsx";
         ActivityLogger::activity($fileName. "تم تصدير ملف  تحت اسم ");
